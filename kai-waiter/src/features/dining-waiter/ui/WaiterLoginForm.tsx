...
import { waiterLoginAction } from "@/features/dining-waiter/actions/waiter.action";
import {
  readAppCompany,
  type AppCompanyConfig,
} from "@/features/company/storage/app-company-storage";
import { loadWaiterSession, saveWaiterSession } from "@/lib/app-session";

const POST_LOGIN_PATH = "/salon";
...
export function WaiterLoginForm() {
  ...
  const [appCompany, setAppCompany] = useState<AppCompanyConfig | null>(null);

  useEffect(() => {
    const session = loadWaiterSession();
    setAppCompany(readAppCompany());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!appCompany) return;
    setError("");
    setSubmitting(true);

    try {
      const session = await waiterLoginAction({
        userName,
        password,
      ...
      >
        <div className="w-full max-w-md rounded-xl border border-border/70 bg-background/80 p-8 shadow-lg backdrop-blur-md">
          <div className="mb-4 flex items-center gap-3">
            <UtensilsCrossed className="h-6 w-6 shrink-0 text-secondary" aria-hidden />
            <h1 className="text-lg font-semibold">Mesero no configurado</h1>
          </div>
  ...
  const companyLabel =
    appCompany.nombreFantasia?.trim() || appCompany.razonSocial.trim();

  return (
    <div className="flex min-h-screen items-center justify-center p-4" data-test-id="login-root">
      <div className="w-full max-w-md rounded-xl border border-border/70 bg-background/80 p-8 shadow-lg backdrop-blur-md">
        <div className="mb-6 flex flex-col items-center text-center">
          <img src="/logo.png" alt="KaiFood" className="h-20 w-20 object-contain" />
