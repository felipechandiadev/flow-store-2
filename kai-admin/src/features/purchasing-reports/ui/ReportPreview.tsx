...
export function ReportPreview({ result, companyLabel }: Props) {
  if (!result) {
    return (
      <div
        ...
        data-test-id="purchasing-report-empty"
        aria-hidden
      />
    );
  }
  ...
  return (
    <div
      className="purchasing-report-print-root rounded-xl border border-border bg-background p-5 shadow-sm print:border-0 print:p-0 print:shadow-none"
      data-test-id="purchasing-report-preview"
    >
      <header className="mb-5 border-b border-border pb-4 print:border-neutral-300">
        <p className="text-xs uppercase tracking-wide text-muted-foreground">
          {companyLabel || "Kai Admin"} · Reportes de compras
        </p>
