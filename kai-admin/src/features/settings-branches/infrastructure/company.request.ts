...
export type CompanyDetails = {
  ...
  isActive: boolean;
  /** Vertical comercial de la empresa (`kaistore` | `kaifood` | `kaiservices`). */
...
function mapCompanyResponse(data: CompanyApiResponse): CompanyDetails | null {
  ...
  return {
    ...
    kaiProduct: normalizeKaiProduct(data.kaiProduct),
...
function normalizeKaiProduct(
  raw: string | null | undefined,
): "kaistore" | "kaifood" | "kaiservices" {
  const v = (raw ?? "").trim().toLowerCase();
  if (v === "kaifood" || v === "kaiservices") return v;
  return "kaistore";
}
