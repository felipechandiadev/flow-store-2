...
export class SalesReportsRequest {
  static async listCatalog(): Promise<SalesReportCatalogItem[]> {
    const headers = await authHeaders();
    const res = await fetch(apiUrl("sales-reports"), {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    return (await res.json()) as SalesReportCatalogItem[];
  }
  ...
  static async run(
    ...
    const headers = await authHeaders();
    const res = await fetch(apiUrl(`sales-reports/${encodeURIComponent(reportId)}/run`), {
      method: "POST",
      headers,
    ...
    return (await res.json()) as SalesReportRunResult;
  }
}
