...
import type { MenuHeroSlideRow } from "../types/hero-slide.types";
...
export class MenuHeroSlidesRequest {
  static async list(): Promise<MenuHeroSlideRow[]> {
    const res = await fetch(apiUrl("/menu/admin/hero-slides"), {
      headers: await authHeaders(),
      cache: "no-store",
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const rows = (await res.json()) as MenuHeroSlideRow[];
    return rows.map((row) => ({
      ...row,
  ...
  static async create(
    body: Omit<MenuHeroSlideRow, "id" | "imageUrl" | "sortOrder"> & {
      sortOrder?: number;
    ...
    const slide = (await res.json()) as MenuHeroSlideRow;
    return { success: true as const, slide };
  }

  static async update(id: string, body: Partial<Omit<MenuHeroSlideRow, "id" | "imageUrl">>) {
    const res = await fetch(apiUrl(`/menu/admin/hero-slides/${id}`), {
      method: "PATCH",
      headers: await authHeaders(),
      body: JSON.stringify(body),
    });
  ...
  static async getSliderSettings() {
    const res = await fetch(apiUrl("/menu/admin/hero-slider-settings"), {
      headers: await authHeaders(),
      cache: "no-store",
    });
    if (!res.ok) {
  ...
  static async reorderOrder(orderedIds: string[]) {
    const res = await fetch(apiUrl("/menu/admin/hero-slides/order"), {
      method: "PUT",
      headers: await authHeaders(),
    ...
    const slides = (await res.json()) as MenuHeroSlideRow[];
    return {
