...
import type {
  ...
  DiningReportSummaryDelta,
} from "@/features/kaifood-reports/types/kaifood-report.types";

type Props = {
  result: DiningReportRunResult | null;
  companyLabel?: string;
};
  ...
  return (
    <span className="relative inline-flex shrink-0">
      <IconButton
        ...
        data-test-id="kaifood-report-kpi-info"
      />
      {open ? (
        <span
          id={tipId}
...
function DeltaBadge({ delta }: { delta: DiningReportSummaryDelta }) {
...
export function ReportPreview({ result, companyLabel }: Props) {
  if (!result) {
    return (
      <div
        ...
        data-test-id="kaifood-report-empty"
        aria-hidden
      />
    );
  }
  ...
  return (
    <div
      className="kaifood-report-print-root rounded-xl border border-border bg-background p-5 shadow-sm print:border-0 print:p-0 print:shadow-none"
      data-test-id="kaifood-report-preview"
    >
      <header className="mb-5 border-b border-border pb-4 print:border-neutral-300">
        <p className="text-xs uppercase tracking-wide text-muted-foreground">
          {companyLabel || "Kai Admin"} · Reportes KaiFood
        </p>
      ...
      {bulletEntries.length > 0 ? (
        <section
          ...
          data-test-id="kaifood-report-kpi-bullets"
        >
          ...
          {cardEntries.map(([key, value]) => {
            ...
            return (
              <div
                ...
                data-test-id={`kaifood-report-kpi-${key}`}
              >
