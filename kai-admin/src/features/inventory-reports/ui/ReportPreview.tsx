...
import { inventoryReportKpiHelp } from "@/features/inventory-reports/lib/report-kpi-help";
import type {
  InventoryReportRunResult,
  InventoryReportSummaryDelta,
  ...
  return (
    <span className="relative inline-flex shrink-0">
      <IconButton
        ...
        data-test-id="inventory-report-kpi-info"
      />
      {open ? (
        <span
          id={tipId}
...
function DeltaBadge({ delta }: { delta: InventoryReportSummaryDelta }) {
    ...
    >
      ...
      {bulletEntries.length > 0 ? (
        <section
          ...
          data-test-id="inventory-report-kpi-bullets"
        >
          {bulletEntries.map(([key, value]) => {
            const help = inventoryReportKpiHelp(key);
            ...
            return (
              <KpiBulletChart
                ...
                testId={`inventory-report-kpi-${key}`}
              />
            );
          })}
        </section>
          ...
          {cardEntries.map(([key, value]) => {
            ...
            return (
              <div
                ...
                data-test-id={`inventory-report-kpi-${key}`}
              >
