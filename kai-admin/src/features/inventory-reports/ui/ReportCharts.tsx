...
import type { InventoryReportSeries } from "@/features/inventory-reports/types/inventory-report.types";
...
function isCountSeries(series: InventoryReportSeries): boolean {
  const t = `${series.id} ${series.label}`.toLowerCase();
  return /(qty|cantidad|count|alerta|transfer|ajuste|sku)/.test(t);
}
...
function SingleChart({ series }: { series: InventoryReportSeries }) {
