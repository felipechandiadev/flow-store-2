    ...
    <div
      className={`${
        variante === "autocomplete"
          ...
          : "fs-text-field"
      } ${showStaticLabel ? "flex min-w-0 flex-col gap-1" : ""}`.trim()}
    >
      {showStaticLabel ? (
        <label
      ...
      <div
        className={`relative ${autocompleteCompact ? "flex min-h-0 min-w-0 flex-1 items-center" : ""} ${className}`.trim()}
      ...
      {isTextArea ? (
        ...
        <div className={`relative ${autocompleteCompact ? "flex min-h-0 w-full flex-1 items-center" : ""}`}>
          <input
            id={inputDomId}
            ref={inputRef}
            type={
            ...
            onKeyDown={onKeyDown}
            className={`${placeholderClassRef.current ?? ""} fs-text-field__input ${compactInputClass} ${borderlessInputClass} ${autocompleteCompact ? "fs-text-field__input--autocomplete-trigger" : ""} block ${isCompact ? "min-w-0" : "min-w-[180px]"} w-full ${startPaddingClass} ${hasEndAdornment ? " pr-3" : (hasEndSymbol || hasPasswordToggle) ? " pr-10" : " pr-3"} ${variantInput} ${disabledStyles} ${comboReadOnlyCursor} z-0`}
            placeholder={
              type === "datePicker" ? `Ej: ${new Date().getFullYear()}` :
              (required ? "" : (shrink || !showPlaceholder ? "" : (placeholder ?? label)))
            }
