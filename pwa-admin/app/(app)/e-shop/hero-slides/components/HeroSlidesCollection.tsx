...
type HeroSlidesCollectionProps = {
  initialSlides: EShopHeroSlideRow[];
};

function sortSlides(slides: EShopHeroSlideRow[]): EShopHeroSlideRow[] {
  return [...slides].sort((a, b) => a.sortOrder - b.sortOrder || a.id.localeCompare(b.id));
}

function applyOrder(slides: EShopHeroSlideRow[], orderedIds: string[]): EShopHeroSlideRow[] {
  const byId = new Map(slides.map((s) => [s.id, s]));
  return orderedIds.map((id, index) => {
    const slide = byId.get(id);
    if (!slide) return null;
    return { ...slide, sortOrder: index + 1 };
  }).filter((s): s is EShopHeroSlideRow => s != null);
}

function displaySortOrder(slide: EShopHeroSlideRow, index: number): number {
...
export function HeroSlidesCollection({ initialSlides }: HeroSlidesCollectionProps) {
  ...
  return (
    <CollectionPageLayout
      ...
      data-test-id="hero-slides-collection"
    />
  );
}
