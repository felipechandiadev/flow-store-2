...
export function EmployeeDetailBankAccountsSection({
  personId,
  loading,
}: {
  personId: string;
  ...
  if (error) {
    return (
      <p className="text-sm text-error" role="alert" data-test-id="employee-detail-bank-error">
        {error}
      </p>
    );
  }
  ...
  if (!personId.trim()) {
    return (
      <p className="text-sm text-muted-foreground" data-test-id="employee-detail-bank-no-person">
        No hay persona asociada al empleado.
      </p>
    );
  }
