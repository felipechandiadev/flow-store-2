...
import type { CustomerBackorderRow } from "@/features/sales-customers/types/customer-related-documents.types";
import { fmtClp, formatCustomerDateTime, TX_TYPE_LABEL } from "./customer-detail-format";

function normalizeRow(raw: Record<string, unknown>): CustomerBackorderRow | null {
...
export function CustomerDetailBackordersSection({ customerId }: { customerId: string }) {
  const [rows, setRows] = useState<CustomerBackorderRow[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void getCustomerBackordersListAction(customerId).then((res) => {
      if (cancelled) return;
      setLoading(false);
      if (res.success) {
        setRows(
          res.rows.map((r) => normalizeRow(r)).filter((x): x is CustomerBackorderRow => x != null),
        );
      } else {
        setError(res.error);
        setRows([]);
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">No hay encargos registrados para este cliente.</p>;
  }

  return (
    <div className="overflow-auto rounded-lg border border-border" data-test-id="customer-detail-backorders">
          ...
          {rows.map((r) => {
            ...
            return (
              <tr key={r.id} className="border-b border-border/80">
                ...
                <td className="px-3 py-2 text-muted-foreground">{formatCustomerDateTime(r.createdAt)}</td>
              </tr>
            );
          })}
        </tbody>
