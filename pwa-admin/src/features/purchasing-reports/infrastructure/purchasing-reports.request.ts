...
export class PurchasingReportsRequest {
  static async listCatalog(): Promise<PurchasingReportCatalogItem[]> {
    const headers = await authHeaders();
    const res = await fetch(apiUrl("purchasing-reports"), {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    return (await res.json()) as PurchasingReportCatalogItem[];
  }
  ...
  static async run(
    ...
    const res = await fetch(
      apiUrl(`purchasing-reports/${encodeURIComponent(reportId)}/run`),
      {
        method: "POST",
        headers,
    ...
    return (await res.json()) as PurchasingReportRunResult;
  }
}
