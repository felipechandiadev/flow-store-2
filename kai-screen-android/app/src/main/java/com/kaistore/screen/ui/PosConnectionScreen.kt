...
import com.kaistore.screen.service.WssLocalProbe
import com.kaistore.screen.ui.prefs.ServiceSetupPrefs
...
fun PosConnectionScreen() {
    ...
    var demoSent by remember { mutableStateOf(false) }
    ...
    fun sendDisplayDemo() {
        ...
        DisplayStateHolder.setSnapshot(
            CustomerDisplaySnapshot(
                state = "active_sale",
                pointOfSaleId = "demo",
                storeName = context.getString(R.string.app_name),
                lines = listOf(
                    CustomerDisplayLine(
                        lineId = "demo",
    ...
    val operativo = readyForPos && displayAttached && posConnected
    ...
    Column(
        ...
        SetupStepCard(
            step = if (wssEnabled) 4 else 3,
            title = stringResource(R.string.pos_step_display_title),
            done = displayAttached,
            status = if (displayAttached) {
                stringResource(R.string.display_attached)
            } else {
                stringResource(R.string.display_missing)
            },
        ) {
            Text(stringResource(R.string.pos_step_display_body), style = MaterialTheme.typography.bodySmall)
            Spacer(Modifier.height(8.dp))
            OutlinedButton(
                onClick = { sendDisplayDemo() },
                modifier = Modifier.fillMaxWidth(),
            ) {
                Text(stringResource(R.string.pos_step_display_test))
            }
            if (demoSent) {
                Text(
                    stringResource(R.string.pos_step_display_test_sent),
                    style = MaterialTheme.typography.bodySmall,
        ...
        SetupStepCard(
            step = if (wssEnabled) 5 else 4,
            title = stringResource(R.string.pos_step_pos_title),
            done = posConnected,
            status = if (posConnected) {
                stringResource(R.string.pos_connected)
            } else {
                stringResource(R.string.pos_disconnected)
            },
        ) {
            Text(stringResource(R.string.pos_step_pos_body), style = MaterialTheme.typography.bodySmall)
        }
