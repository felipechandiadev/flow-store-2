...
import {
  readDeliveryCompany,
  writeDeliveryCompany,
} from "@/features/company/storage/delivery-company-storage";

type DeliverySetupClientProps = {
  companies: PublicCompany[];
  initialError?: string | null;
};

export function DeliverySetupClient({
  companies,
  initialError,
  ...
  useEffect(() => {
    const current = readDeliveryCompany();
    if (current?.id) {
  ...
  return (
    <div
      ...
      data-test-id="delivery-setup-root"
    >
      <div className="fs-dialog__paper w-full max-w-lg overflow-hidden shadow-md">
        <div className="fs-dialog__header flex items-start gap-2 border-b border-border/70 px-4 pb-3 pt-4">
          <IconButton
            ...
            data-test-id="delivery-setup-back"
            disabled={saving}
          />
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Elige la empresa a la que se conectará este dispositivo. La
              selección se guarda localmente hasta que la cambies aquí.
            </p>
          </div>
        </div>
        ...
        <div className="px-4 pb-4 pt-3">
          {savedAt ? (
            <p
              ...
              data-test-id="delivery-setup-current"
            >
          ...
          {error ? (
            <p
              ...
              data-test-id="delivery-setup-error"
            >
              {error}
            </p>
          ) : null}
          ...
          {sortedCompanies.length === 0 ? (
            <div
              ...
              data-test-id="delivery-setup-empty"
            >
            ...
            <ul
              ...
              role="listbox"
              data-test-id="delivery-setup-list"
            >
                ...
                return (
                  <li key={c.id}>
                    <button
                      ...
                      aria-pressed={active}
                      data-test-id={`delivery-setup-option-${c.id}`}
                    >
        ...
        <div
          ...
          data-test-id="delivery-setup-actions"
          role="group"
          aria-label="Acciones"
        >
          <Button
            ...
            onClick={handleCancel}
            disabled={saving}
            data-test-id="delivery-setup-cancel"
          >
            Cancelar
          </Button>
          <Button
            ...
            disabled={!selectedId}
            loading={saving}
            data-test-id="delivery-setup-save"
          >
            Guardar
          </Button>
        </div>
