...
import type { TaxListItem } from "@/features/purchasing-reception/types/tax.types";
import type {
  CreatePurchaseOrderInput,
  CreatePurchaseOrderResult,
} from "@/features/purchasing-reception/types/purchase-order.types";
import type { CreatePurchaseOrderLineInput } from "@/features/purchasing-reception/types/purchase-order.types";
import type {
...
} from "@/features/purchasing-reception/types/purchase-return.types";
import type {
  PurchasingTransactionDetail,
  PurchasingTransactionDetailResult,
} from "@/features/purchasing-reception/types/purchasing-detail.types";
import {
...
export type PurchaseDocumentBuilderProps = {
  ...
  /** POS: efectivo desde sesión de caja; admin: centro de acopio. */
  paymentCashContext?: "admin_cash_hub" | "pos_cash_session";
};
...
export function PurchaseDocumentBuilder({
  ...
  const reference = usePosPurchaseDocumentReferenceData();
  ...
  useEffect(() => {
    ...
    void getCompanyDetailsPosAction()
      .then((d) => {
        if (!cancelled) {
    ...
    async (saveAsDraft: boolean) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          try {
            const companyDetails = await getCompanyDetailsPosAction();
