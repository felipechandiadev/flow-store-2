...
function posAppLabel(productLabel: string, posKind?: PosKind | null): string {
  return posKind === "PRESALE" ? `${productLabel} Preventa` : `${productLabel} POS`;
}

export default function PosTopBar({
  pointOfSaleName = null,
  companyTradeName = null,
  companyKaiProduct = null,
  personName = null,
  userRole = null,
}: PosTopBarProps) {
  ...
  const title = resolveTopbarProductLabel(companyKaiProduct);
  ...
  const printService = usePrintServiceConnection({
    ...
    appLabel: posAppLabel(title, posKindFromClient),
