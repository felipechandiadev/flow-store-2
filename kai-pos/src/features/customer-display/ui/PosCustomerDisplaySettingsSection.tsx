...
export function PosCustomerDisplaySettingsSection({
  ...
  const onTestDisplay = async () => {
    ...
    if (enabled && agentStatus && !agentStatus.displayAttached) {
      setTestMessage(
        "Aviso: Kai CFD no detecta pantalla secundaria. La prueba se enviará igual; revise el cable HDMI o el módulo dual-screen.",
      );
    }
    ...
    try {
      ...
      if (!conn) {
        setTestMessage("Active Kai CFD en el POS y verifique que el punto de venta esté configurado.");
        return;
      }
      ...
      setTestMessage("No se pudo enviar la prueba. Verifique que Kai CFD esté activo y el certificado WSS confiado.");
    } finally {
      setTestBusy(false);
    }
  };
  ...
  return (
    <section className={`rounded-xl border border-border bg-background p-4 ${className}`.trim()}>
      <h2 className="text-base font-semibold text-foreground">Visor de cliente (Kai CFD)</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        Muestra el carrito al cliente en la segunda pantalla de la tablet. Requiere la app Kai CFD en
        el mismo equipo.
      </p>
      ...
      <div className="mt-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm font-medium">Activar visor de cliente</span>
          <Switch
            checked={enabled}
            onChange={(v) => {
              setEnabled(v);
        ...
        <div className="rounded-lg border border-border bg-muted/20 p-3">
          ...
          <ul className="mt-2 space-y-1.5" data-test-id="kai-screen-status-checklist">
            <StatusCheckItem ok={enabled} label="Kai CFD activado en el POS" />
            <StatusCheckItem
              ok={agentConnected}
              label={
                agentConnected
                  ? "Agente Kai CFD conectado"
                  : enabled
                    ? "Agente Kai CFD desconectado"
                    : "Agente Kai CFD (desactivado)"
              }
            />
            <StatusCheckItem
              ok={displayAttached}
