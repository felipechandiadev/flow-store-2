...
export class DiningPosRequest {
  ...
  static async markFireReady(
    ...
    if (!unitId) {
      return { success: false, message: "productionUnitId es obligatorio" };
    }

    const res = await backendFetch(
      `${base}/api/dining/orders/${encodeURIComponent(orderId.trim())}/fires/${encodeURIComponent(fireId.trim())}/ready`,
      {
        method: "POST",
        headers: auth.headers,
        body: JSON.stringify({ productionUnitId: unitId }),
      },
    );
    if (!res) return { success: false, message: BACKEND_CONNECTION_MESSAGE };
    const data = await res.json().catch(() => null);
  ...
  static async markFireReadyForPickup(
    orderId: string,
    fireId: string,
  ): Promise<PosDiningMutationResponse> {
    const base = process.env.BACKEND_API_URL;
    ...
    const res = await backendFetch(
      `${base}/api/dining/orders/${encodeURIComponent(orderId.trim())}/fires/${encodeURIComponent(fireId.trim())}/ready-for-pickup`,
      {
        method: "POST",
        headers: auth.headers,
        body: JSON.stringify({}),
  ...
  static async markFireDelivered(
    orderId: string,
    fireId: string,
  ): Promise<PosDiningMutationResponse> {
    const base = process.env.BACKEND_API_URL;
    ...
    const res = await backendFetch(
      `${base}/api/dining/orders/${encodeURIComponent(orderId.trim())}/fires/${encodeURIComponent(fireId.trim())}/delivered`,
      {
        method: "POST",
        headers: auth.headers,
        body: JSON.stringify({}),
  ...
  static async reopenOrder(orderId: string): Promise<PosDiningMutationResponse> {
    const base = process.env.BACKEND_API_URL;
    if (!base) return { success: false, message: "BACKEND_API_URL no está configurada" };
    const auth = await authHeaders();
    if (!auth.ok) return { success: false, message: auth.message };
    ...
    const res = await backendFetch(
      `${base}/api/dining/orders/${encodeURIComponent(orderId.trim())}/reopen`,
      { method: "POST", headers: auth.headers },
    );
    if (!res) return { success: false, message: BACKEND_CONNECTION_MESSAGE };
    const data = await res.json().catch(() => null);
  ...
  static async abandonEmptyOrder(orderId: string): Promise<PosDiningMutationResponse> {
    const base = process.env.BACKEND_API_URL;
    if (!base) return { success: false, message: "BACKEND_API_URL no está configurada" };
    const auth = await authHeaders();
    if (!auth.ok) return { success: false, message: auth.message };
    ...
    const res = await backendFetch(
      `${base}/api/dining/orders/${encodeURIComponent(orderId.trim())}/abandon-empty`,
      { method: "POST", headers: auth.headers },
    );
    if (!res) return { success: false, message: BACKEND_CONNECTION_MESSAGE };
    const data = await res.json().catch(() => null);
