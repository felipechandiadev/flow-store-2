...
export class DeliverySchemaBootstrap implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      if (postgisReady) {
        ...
        if (columnsOk) {
          this.logger.log('Delivery schema bootstrap OK (PostGIS + geom)');
        } else {
          this.logger.error(
      ...
      this.logger.warn(`Delivery schema bootstrap skipped: ${String(err)}`);
    }
  }

  /** Renombra `e_shop_delivery_*` → `delivery_*` en bases ya provisionadas.
   * Si coexisten ambas (bootstrap creó tablas vacías antes del rename),
   * reemplaza las nuevas con los datos legacy.
   */
  private async renameLegacyTables() {
    // Orden: dependientes primero (FKs lógicas / CASCADE).
  ...
  private async ensureCoreTables() {
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_coverage_communes" (
        ...
        CONSTRAINT "PK_delivery_coverage_communes" PRIMARY KEY ("id")
      );
    `);

    await this.dataSource.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS "uq_delivery_communes_company_code"
      ON "delivery_coverage_communes" ("company_id", "code");
    `);
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_occurrences" (
        ...
        "created_at" TIMESTAMP NOT NULL DEFAULT now(),
        "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
        CONSTRAINT "PK_delivery_occurrences" PRIMARY KEY ("id")
      );
    `);

    await this.dataSource.query(`
      ALTER TABLE "delivery_occurrences"
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_occurrence_zones" (
        "occurrence_id" uuid NOT NULL,
        "zone_id" uuid NOT NULL,
        "company_id" uuid NOT NULL,
        CONSTRAINT "PK_delivery_occurrence_zones" PRIMARY KEY ("occurrence_id", "zone_id")
      );
    `);
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_orders" (
        ...
        "source_channel" varchar(16) NOT NULL DEFAULT 'ESHOP',
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_dispatches" (
        ...
        "created_at" TIMESTAMP NOT NULL DEFAULT now(),
        "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
        CONSTRAINT "PK_delivery_dispatches" PRIMARY KEY ("id")
      );
    `);
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_stops" (
        ...
        "created_at" TIMESTAMP NOT NULL DEFAULT now(),
        "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
        CONSTRAINT "PK_delivery_stops" PRIMARY KEY ("id")
      );
    `);
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_settings" (
        ...
        "created_at" TIMESTAMP NOT NULL DEFAULT now(),
        "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
        CONSTRAINT "PK_delivery_settings" PRIMARY KEY ("company_id")
      );
    `);

    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS "delivery_order_line_picks" (
        "id" uuid NOT NULL DEFAULT gen_random_uuid(),
        "company_id" uuid NOT NULL,
        "delivery_order_id" uuid NOT NULL,
        ...
        "created_at" TIMESTAMP NOT NULL DEFAULT now(),
        "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
        CONSTRAINT "PK_delivery_order_line_picks" PRIMARY KEY ("id")
      );
    `);
    await this.dataSource.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS "uq_delivery_order_line_picks"
      ON "delivery_order_line_picks" ("company_id", "delivery_order_id", "transaction_line_id");
    `);
    await this.dataSource.query(`
      CREATE INDEX IF NOT EXISTS "idx_delivery_order_line_picks_order"
      ON "delivery_order_line_picks" ("company_id", "delivery_order_id");
    `);

    await this.dataSource.query(`
      ALTER TABLE "delivery_orders"
      ADD COLUMN IF NOT EXISTS "source_channel" varchar(16) NOT NULL DEFAULT 'ESHOP';
    `);
  }
  ...
  private async ensureSpatialIndexes() {
    ...
    `);
    await this.dataSource.query(`
      CREATE INDEX IF NOT EXISTS "idx_delivery_orders_delivery_point_gist"
      ON "delivery_orders" USING GIST ("delivery_point");
    `);
  }
}
