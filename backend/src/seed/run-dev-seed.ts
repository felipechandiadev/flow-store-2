...
/** Almacén legacy Parabrisas; se elimina si quedó de corridas anteriores. */
const SEED_REMOVED_STORAGE_CODE = 'SEED-DEP-PRINCIPAL';

const SEED_UNIT_BASE_NAME = 'Unidad';
const SEED_UNIT_BASE_SYMBOL = 'un';

const SEED_REMOVED_UNIT_SYMBOLS = ['doc'] as const;
...
const LEGACY_SEED_ATTRIBUTE_NAMES = ['AÑO', 'MARCA DE AUTO'] as const;
...
function resolveSeedVariantAttributeValues(
  ...
  for (const [attrName, optionValue] of Object.entries(raw)) {
    ...
    if (!attr?.id) {
      console.warn(
        `⚠️ Seed dev: atributo «${attrName}» no encontrado; se omite en variante`,
      );
      continue;
    }
    ...
    if (!allowed.includes(option)) {
      console.warn(
        `⚠️ Seed dev: opción «${option}» no válida para atributo «${attrName}»; se omite`,
      );
      continue;
    }
    out[attr.id] = option;
...
async function bootstrap() {
   ...
   */
    ...
    // Categorías de producto (desarrollo genérico)
    ...
    for (let i = 0; i < SEED_DEV_CATEGORIES.length; i++) {
      const name = SEED_DEV_CATEGORIES[i];
    ...
    // Catálogo desarrollo (~20 variantes, multi-rubro)
    ...
    for (const nm of SEED_DEV_BRANDS) {
      ...
      if (!b) {
        b = brandRepo.create({
          companyId: company.id,
          name: nm,
          description: 'Marca catálogo desarrollo',
          isActive: true,
        });
        b = await brandRepo.save(b);
        console.log(`✅ Marca creada: «${nm}» id=${b.id}`);
      } else {
    ...
    const devStockByVariantId = new Map<string, number>();
    let devVariantCount = 0;

    for (const def of SEED_DEV_PRODUCTS) {
      const category = categoryByName.get(def.categoryName);
      if (!category) {
        throw new Error(
          `Seed dev: categoría «${def.categoryName}» no existe (producto «${def.name}»)`,
        );
      }

      let product = await productRepo.findOne({
        where: { name: def.name, companyId: company.id },
      });
      const productPayload = {
        name: def.name,
        brand: def.brand,
        brandId: brandIdByName.get(def.brand) ?? null,
      ...
      for (const vd of def.variants) {
        let variant = await variantRepo.findOne({
          where: { sku: vd.sku, companyId: company.id },
        });
        ...
        if (vd.trackInventory) {
          devStockByVariantId.set(savedVariant.id, 12);
        }
      }

      console.log(
        `✅ Producto dev: «${def.name}» (${def.productType}) variantes=${def.variants.length}`,
      );
    }
