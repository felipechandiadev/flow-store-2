...
describe('Receptions - Accounting Integration (Diagnóstico)', () => {
  ...
  describe('📊 DIAGNÓSTICO: Sistema Actual', () => {
    ...
    it('❌ NO incluye metadata de pagos programados (numberOfInstallments)', async () => {
      ...
      // Assert
      const callArgs =
        mockTransactionsService.createTransaction.mock.calls[0][0];
    ...
    it('📋 RESUMEN: Lo que SÍ y NO está funcionando', async () => {
      ...
      console.log(
        '   3. ✓ La transacción dispara evento "transaction.created"',
      );
      console.log('   4. ✓ El AccountingEngineListener escucha el evento');
      console.log(
        '   5. ✓ Existe regla contable PURCHASE en accounting-rules.json:',
      );
      console.log('      - DÉBITO: Inventario (cuenta 1.3.01)');
      console.log(
        '      - CRÉDITO: Proveedores/Cuentas por Pagar (cuenta 2.1.01)',
      );
      ...
      console.log(
        '   5. ✗ NO hay seguimiento de pagos programados al proveedor',
      );
      ...
      console.log(
        '   3. El CreateInstallmentsListener los detectará automáticamente',
      );
      console.log('   4. Se crearán las cuotas en la tabla installments');
      console.log(
        '   5. Se puede hacer seguimiento de pagos pendientes a proveedores',
      );
      ...
      console.log(
        '   - Las cuotas (installments) son para seguimiento, no contabilidad',
      );
      console.log(
        '   - Cuando se pague una cuota, se crea transacción SUPPLIER_PAYMENT',
      );
      console.log(
        '\n═══════════════════════════════════════════════════════\n',
      );
  ...
  describe('📚 REFERENCIA: Regla Contable PURCHASE', () => {
    it('documenta la regla contable que procesa las recepciones', () => {
      ...
      console.log(
        '  CRÉDITO: Cuentas por Pagar Proveedores (Pasivo) ← aumenta',
      );
      console.log('\nCUANDO SE ACTIVA:');
