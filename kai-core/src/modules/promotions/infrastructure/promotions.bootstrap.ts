...
/**
 * Bootstrap idempotente del sistema de promociones.
 *
 * En entornos de desarrollo no siempre se corre `typeorm migration:run`,
 * por lo que aseguramos que las 9 tablas (`promotions` +
 * 7 `promotion_scope_*` + `promotion_redemptions`) y sus 4 enums existan
 * al iniciar la app. Idempotente: solo crea lo que falta. Para entornos
 * productivos, la migración explícita
 * `1748000000000-Promotions.ts` es la fuente de verdad.
 */
@Injectable()
export class PromotionsBootstrap implements OnModuleInit {
  private readonly logger = new Logger(PromotionsBootstrap.name);
  ...
  async onModuleInit(): Promise<void> {
    try {
      await this.ensureEnums();
      await this.ensurePromotionsTable();
      await this.ensureScopeTables();
      await this.ensureRedemptionsTable();
    } catch (err) {
      this.logger.error(
        `No se pudo asegurar el schema de promotions: ${
  ...
  private async ensureEnums(): Promise<void> {
  ...
  private async ensurePromotionsTable(): Promise<void> {
    await this.dataSource.query(
  ...
  private async ensureScopeTables(): Promise<void> {
  ...
  private async ensureRedemptionsTable(): Promise<void> {
    await this.dataSource.query(
