...
export type DiningReportHandlerContext = {
  companyId: string;
  params: Record<string, unknown>;
};

export interface DiningReportHandler {
  readonly id: string;
  readonly title: string;
  readonly description: string;
  readonly wave: 'mvp' | 'p1';
  validate(params: Record<string, unknown>): Record<string, unknown>;
  run(ctx: DiningReportHandlerContext): Promise<DiningReportRunResult>;
}

export type ReportGranularity = 'day' | 'week' | 'month';
export type CompareWith = 'none' | 'previousPeriod' | 'samePeriodLastYear';
export type DiningOrderKindFilter = 'TABLE' | 'COUNTER' | 'TAKEAWAY';
...
): Record<string, DiningReportSummaryDelta> {
  const out: Record<string, DiningReportSummaryDelta> = {};
