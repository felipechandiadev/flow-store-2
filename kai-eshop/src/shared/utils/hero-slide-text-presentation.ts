...
export function normalizeHeroSlideHexColor(value: string | null | undefined): string | null {
...
export function heroSlideRelativeLuminance(hex: string): number {
...
export function heroSlideTextShadowForHex(hex: string): string {
  const luminance = heroSlideRelativeLuminance(hex);
...
export function heroSlideIndicatorBoxShadowForHex(hex: string): string {
  const luminance = heroSlideRelativeLuminance(hex);
  const isLightText = luminance > 0.55;
  if (isLightText) {
...
export type HeroSlideTextPresentation = {
  ...
  indicatorStyle: (active: boolean) => CSSProperties;
};

/** Puntos (inactivo) y barra (activo) del carrusel hero — mismo color que el título del slide. */
export function heroSlideCarouselIndicatorStyle(
  ...
  if (hex) {
    return {
      backgroundColor: active ? hex : hexToRgba(hex, 0.4),
      boxShadow: heroSlideIndicatorBoxShadowForHex(hex),
    };
  }
  return {
    backgroundColor: active ? "var(--fs-foreground)" : "var(--fs-foreground)",
...
export function getHeroSlideTextPresentation(
  ...
  const indicatorShadow = heroSlideIndicatorBoxShadowForHex(hex);
