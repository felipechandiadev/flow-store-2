# Inventario de imágenes seed (clasificado)

## `products/` — imagen principal por producto (15)

| Archivo | Producto en catálogo |
|---------|----------------------|
| `aceite-oliva-extra-virgen.png` | Aceite de oliva extra virgen |
| `cafe-molido-premium.png` | Café molido premium |
| `harina-integral.png` | Harina integral |
| `galletas-surtidas.png` | Galletas surtidas |
| `te-verde-caja-20-bolsitas.png` | Té verde caja 20 bolsitas |
| `polera-algodon.png` | Polera algodón |
| `calcetines-deportivos.png` | Calcetines deportivos (pack colores) |
| `detergente-liquido-3l.png` | Detergente líquido 3 L |
| `toalla-bano-algodon.png` | Toalla baño algodón |
| `mouse-inalambrico.png` | Mouse inalámbrico |
| `cable-hdmi-2m.png` | Cable HDMI 2 m |
| `cuaderno-universitario-100-hojas.png` | Cuaderno universitario 100 hojas |
| `mochila-urbana.png` | Mochila urbana |
| `servicio-armado-pedido.png` | Servicio armado de pedido |
| `pack-plantillas-hoja-calculo.png` | Pack plantillas hoja de cálculo |

## `variants/` — calcetines por SKU (3, color Gris)

| Archivo | Variante |
|---------|----------|
| `SEED-DEV-CAL-S-GRA.png` | Calcetines S · Gris |
| `SEED-DEV-CAL-M-GRA.png` | Calcetines M · Gris |
| `SEED-DEV-CAL-L-GRA.png` | Calcetines L · Gris |
