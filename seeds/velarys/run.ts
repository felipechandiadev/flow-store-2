#!/usr/bin/env ts-node
/** Seed Velarys — KaiFood mínimo. `npm run seed:velarys` */
...
import {
  ...
  SEED_VELARYS_COMPANY,
  SEED_VELARYS_PAYMENTS,
  SEED_VELARYS_USERS,
...
import {
  loadVelarysCatalog,
  mapVelarysCatalogToSeedProducts,
  type VelarysProductionUnitCode,
} from './load-catalog';
...
async function ensureCompany(
  ...
  const def = SEED_VELARYS_COMPANY;
  ...
  const paymentCatalog = buildSeedCompanyPaymentCatalog(SEED_VELARYS_PAYMENTS);
...
async function bootstrap() {
  ...
  try {
    ...
    const catalog = loadVelarysCatalog();
    const { products: seedProducts, productionUnitBySku } =
      mapVelarysCatalogToSeedProducts(catalog, SEED_BRAND_NAME);
      ...
      async () => {
        ...
        if (fresh) {
          fresh.settings = buildSeedCompanySettings({
            ...
            paymentMethods: paymentCatalog,
            kaiProduct: 'kaifood',
            menuPublicSlug: SEED_VELARYS_COMPANY.menuPublicSlug,
            ...
            menuExtras: {
              ...
              menuFindUs: {
                ...
                address: SEED_VELARYS_COMPANY.address,
                phone: SEED_VELARYS_COMPANY.phone,
              },
        ...
        console.log(
          `✅ Sucursal ${site.name}: STORE · POS · salón 8 mesas · tips ON · menú=${SEED_VELARYS_COMPANY.menuPublicSlug}`,
        );
    ...
    await TenantContext.run(
      ...
      async () => {
        ...
        console.log(
          `✅ Socio ${SEED_SHAREHOLDER.firstName} ${SEED_SHAREHOLDER.lastName} 100%`,
        );
        ...
        if (!supplier) {
          supplier = supplierRepo.create({
            ...
            isActive: true,
            notes: 'Proveedor de prueba seed Velarys',
          });
        } else {
        ...
        const ensurePu = async (params: {
          code: VelarysProductionUnitCode;
          name: string;
          purpose: ProductionUnitPurpose;
          kitchenFulfillmentMode: KitchenFulfillmentMode | null;
        }) => {
          let pu = await productionUnitRepo.findOne({
            where: {
              companyId: company.id,
              branchId: branchOps.branchId,
              code: params.code,
          ...
          if (!pu) {
            pu = await productionUnitRepo.save(
              productionUnitRepo.create({
                ...
                kitchenFulfillmentMode:
                  params.kitchenFulfillmentMode ?? KitchenFulfillmentMode.KDS,
            ...
            pu.name = params.name;
            pu.purpose = params.purpose;
            if (params.kitchenFulfillmentMode) {
              pu.kitchenFulfillmentMode = params.kitchenFulfillmentMode;
            }
            pu.inventoryMode = ProductionUnitInventoryMode.DEPENDENT;
            pu.defaultInputStorageId = branchOps.storageId;
            pu.isActive = true;
            pu = await productionUnitRepo.save(pu);
          }
          return pu;
        };

        const barra = await ensurePu({
          code: 'BARRA',
          name: 'Barra',
          purpose: ProductionUnitPurpose.KITCHEN,
          kitchenFulfillmentMode: KitchenFulfillmentMode.KDS,
        });
        const cocina = await ensurePu({
          code: 'COCINA',
          name: 'Cocina',
          purpose: ProductionUnitPurpose.KITCHEN,
          kitchenFulfillmentMode: KitchenFulfillmentMode.PRINTED,
        });
        const pasteleria = await ensurePu({
          code: 'PASTELERIA',
          name: 'Pastelería',
          purpose: ProductionUnitPurpose.BATCH,
          kitchenFulfillmentMode: KitchenFulfillmentMode.KDS,
        });
        const upByCode: Record<VelarysProductionUnitCode, ProductionUnit> = {
          BARRA: barra,
          COCINA: cocina,
          PASTELERIA: pasteleria,
        };
        console.log(
          `✅ UPs: Barra · Cocina (PRINTED) · Pastelería ids=${barra.id.slice(0, 8)}…`,
        );

        const salonLu = await ensureLaborUnitOnBranch({
          code: 'UL-SALON',
          name: 'Salón',
          description: 'Unidad laboral salón Velarys',
        });
        const opsLu = await ensureLaborUnitOnBranch({
          code: 'UL-OPS',
          name: 'Operaciones',
          description: 'UL operaciones (barra/cocina/pastelería)',
        });

        for (const pu of [barra, cocina, pasteleria]) {
          const existing = await laborUnitPuRepo.findOne({
            where: {
              laborUnitId: opsLu.id,
        ...
        let shift = await laborUnitShiftRepo.findOne({
          where: { companyId: company.id, code: 'ULS-VELARYS-JORNADA' },
          withDeleted: true,
        });
        if (!shift) {
          shift = laborUnitShiftRepo.create({
            companyId: company.id,
            laborUnitId: opsLu.id,
            code: 'ULS-VELARYS-JORNADA',
            name: 'Jornada Velarys',
          ...
          shift.laborUnitId = opsLu.id;
          shift.name = 'Jornada Velarys';
          shift.scheduleJson = schedule;
          shift.isActive = true;
          shift.effectiveFrom = '2025-01-01';
        }
        await laborUnitShiftRepo.save(shift);
        console.log(`✅ UL Salón + UL Ops + turno ${shift.code}`);
        ...
        console.log(`✅ Routing variante→UP: ${routed} vínculos nuevos`);

        const meseroPerson = await personRepo.findOne({
          where: {
            documentNumber: SEED_VELARYS_USERS.mesero.documentNumber,
            companyId: company.id,
            deletedAt: IsNull(),
          },
        });
        ...
        if (!employee) {
          ...
          employee.branchId = branchOps.branchId;
        ...
        if (!contract) {
          ...
          contract.branchId = branchOps.branchId;
        }
    ...
    console.log('✅ Seed Velarys OK');
    console.log(
      `   • ${SEED_VELARYS_COMPANY.nombreFantasia} (${SEED_VELARYS_COMPANY.rut}) · kaifood · ${catalog.products.length} productos`,
    );
    console.log(
      `   • Menú ${SEED_VELARYS_COMPANY.menuPublicSlug} · UPs Barra/Cocina/Pastelería · tips ON`,
    );
    console.log(`   • Password seed: ${password}`);
    console.log(
      `   • superadmin / ${password}  · admin / ${password}  · operador / ${password}  · mesero / ${password}`,
    );
  } finally {
    await app.close();
  }
...
bootstrap().catch((err) => {
  console.error('❌ Seed Velarys falló', err);
  process.exit(1);
});
