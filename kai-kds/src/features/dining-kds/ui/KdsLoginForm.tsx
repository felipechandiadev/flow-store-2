...
import { kdsLoginAction } from "@/features/dining-kds/actions/kds.action";
import {
  readAppCompany,
  type AppCompanyConfig,
} from "@/features/company/storage/app-company-storage";
import { loadKdsSession, saveKdsSession } from "@/lib/app-session";

const POST_LOGIN_PATH = "/queue";

export function KdsLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const session = await kdsLoginAction({
        userName,
        password,
      ...
      >
        <div className="w-full max-w-md rounded-xl border border-border/70 bg-background/80 p-8 shadow-lg backdrop-blur-md">
          <div className="mb-4 flex items-center gap-3">
            <ChefHat className="h-6 w-6 shrink-0 text-secondary" aria-hidden />
            <h1 className="text-lg font-semibold">KDS no configurado</h1>
          </div>
