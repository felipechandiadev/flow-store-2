...
Microservicios auxiliares del monorepo: procesos con **ciclo de vida propio** que **Kai Core** (`backend`) consume por HTTP. No son apps instalables de la suite (PWA/nativos).
...
| `services/kai-osrm` | *(pendiente)* Empaquetado ops de OSRM | **5001** | Imagen `osrm-backend` + datos — **no** Nest |
...
```
pwa-eshop / admin  →  backend (Kai Core)
                         │
                         │  KaiMailClient  (HTTP)
...
2. **Cliente delgado en Core** — `backend/src/shared/mail/kai-mail.client.ts`. Si no hay `KAI_MAIL_URL`, no envía (feature off).
...
Referencia rápida: [`services/kai-mail/README.md`](../../services/kai-mail/README.md).

---

## 3. OSRM hoy vs el patrón mail

| | **kai-mail** (`services/`) | **OSRM** (hoy) |
|--|----------------------------|----------------|
| Qué es | App Nest propia (lógica Kai) | Binario **third-party** (`osrm-backend`) |
| Dónde | `services/kai-mail` | Contenedor en `backend/docker-compose.yml` (perfil `osrm`) |
| Datos / bootstrap | — | `backend/osrm-data/`, `backend/scripts/osrm-bootstrap.sh` |
| Cliente en Core | `KaiMailClient` | `OsrmHttpClient` en `backend/src/modules/routing/` |
...
| Enfoque | Cuándo |
|---------|--------|
| Dejar OSRM en `backend/docker-compose` (perfil) | Aceptable solo mientras la migración no esté hecha |
...
**No mezclar** con rename `backend` → `kai-core` ni con `pwa-*` → `kai-*` en el mismo PR.
...
services/kai-osrm/
  ...
  scripts/osrm-bootstrap.sh # Movido o wrapper desde backend/scripts
  data/                     # o volumen documentado (ex backend/osrm-data)
```

Core sigue con:

- `OSRM_URL=http://localhost:5001` (o URL de VPS)
- `backend/src/modules/routing/` sin cambios de contrato

### 5.2 Checklist de implementación

- [ ] Crear `services/kai-osrm/` con compose + README (perfil/`docker compose -f … up`)
- [ ] Mover o reubicar `osrm-bootstrap.sh` y documentar primera carga de mapa (`maule-parral.osrm` u otro extract)
- [ ] Mover / enlazar `osrm-data` (gitkeep + `.gitignore` de binarios grandes si aplica)
- [ ] Quitar (o dejar deprecated con comentario → nuevo path) el servicio `osrm` de `backend/docker-compose.yml`
- [ ] Actualizar `envs/shared.env.example` / README envs: comentario “levantar desde `services/kai-osrm`”
- [ ] Actualizar docs de reparto ([`REPARTO-MULTI-CANAL.md`](../project/REPARTO-MULTI-CANAL.md)) con el nuevo path
...
| ¿OSRM es distinto? | Motor third-party; cliente ya existe; falta mover **ops** a `services/kai-osrm` |
| ¿Hacer Nest “kai-osrm”? | No |
| ¿Estado migración? | **Pendiente** — checklist §5.2 |
