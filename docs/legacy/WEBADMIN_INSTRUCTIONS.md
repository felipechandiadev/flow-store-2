...
- `IconButton` en celdas de acciones: `variant="action"`, `size="sm"`. Ver `pwa-admin/src/shared/components/DataGrid/components/RowActions.tsx`.

### Cards: IconButton
- En **tarjetas** (`Card` y acciones con icono), `IconButton` **solo** `variant="action"` (y `size="sm"`). El `Card` compartido fija el variant en acciones con `icon`/`ariaLabel`.
