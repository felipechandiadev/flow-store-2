...
| **Comercial** | *Kai Suite* | Copy comercial + **repo GitHub** `kai-suite` |
...
| **Repositorio GitHub** | `felipechandiadev/flow-store-2` | **`felipechandiadev/kai-suite`** (rename) | ✅ Hecho (jul 2026) |
| **URL clone** | `…/flow-store-2.git` | `…/kai-suite.git` | ✅ Hecho (local); VPS pendiente |
...
| **Agente Tauri (carpeta)** | `print-service/` | **`kai-printers-desktop/`** | ✅ |
...
| Ámbito | ¿Listo? | Notas |
|--------|---------|-------|
| **F1 — Repo / URL GitHub** | ✅ Local | Rename a `kai-suite` hecho; **VPS** `git remote set-url` pendiente |
| **F2 — kai-printers-desktop** | ✅ | Carpeta renombrada; CI y scripts alineados |
...
|------------|-----------|--------|
| **Rename (recomendado)** | `github.com/…/kai-suite.git` | Mismo repo; GitHub redirige `flow-store-2` un tiempo |
...
|--------|-------------|---------------|
| **A — Rename** | Settings → Rename `flow-store-2` → `kai-suite` | ✅ **Hecho** — conserva issues, PRs, deploy keys |
| **B — Repo nuevo** | Crear repo vacío y push | Solo si se necesita historial limpio |

**Decisión:** ✅ A (rename)  
**Nombre final del repo:** ✅ **`kai-suite`**

**Nota:** “Kai Suite” como marca comercial y slug de repo alineados. No usar `kaistore` solo — el monorepo incluye KaiFood / KaiServices.
...
| Tipo | Patrón | Ejemplo |
|------|--------|---------|
| Plataforma / repo / npm | `kai-suite` (GitHub), `@kai/*` | repo `kai-suite`, `@kai/kai-brand` |
...
**Prerrequisito:** carpeta local `kai-printers-desktop/` en máquinas que compilan Tauri.

| ID | Cambio | Archivos afectados |
|----|--------|-------------------|
| F2.T1 | Renombrar carpeta | `print-service/` → `kai-printers-desktop/` |
...
| F2.T4 | Workflows CI | `.github/workflows/kai-printers-desktop*.yml` |
...
|---------|----------|------------|-------------------|
| **`.github/`** | **Sí — CI activo** | **No** | Workflows: `kai-printers-android`, `kai-screen-android`, `kai-printers-desktop`, `kai-printers-desktop-release`, `scale-service-client` |
...
| `.github/workflows/` | kai-printers-desktop | F2 |
| `packages/kai-printers-release/` | kai-printers-desktop path | F2 |
...
| `.github/workflows/` | kai-printers-desktop naming | F2 (CI **mantener**) |
